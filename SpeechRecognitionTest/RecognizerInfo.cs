...
using Microsoft.Speech.Internal.ObjectTokens;
...
public class RecognizerInfo
{
    ...
    internal string SapiObjectTokenId => _sapiObjectTokenId;
    ...
    private RecognizerInfo(ObjectToken token, CultureInfo culture)
    {
        ...
        foreach (string text in valueNames)
        {
            ...
            {
                _attributes.InternalDictionary[text] = value;
            }
        }
    ...
    internal static RecognizerInfo FindBestMatch(string requiredAttributes, string optionalAttributes)
    {
        ObjectToken objectToken = ObjectToken.FindBestMatch("HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Speech Server\\v11.0\\Recognizers", requiredAttributes, optionalAttributes);
        if (objectToken == null)
        {
            return null;
        }

        return Create(objectToken);
    }
