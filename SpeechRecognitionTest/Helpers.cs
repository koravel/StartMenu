...
internal static class Helpers
{
    ...
    internal const string _noClassificationResult = "RecognitionResultRejected";
    ...
    internal static bool CompareInvariantCulture(CultureInfo culture1, CultureInfo culture2)
    {
        ...
        while (!culture1.IsNeutralCulture && culture1 != CultureInfo.InvariantCulture)
        {
            culture1 = culture1.Parent;
        }

        while (!culture2.IsNeutralCulture && culture2 != CultureInfo.InvariantCulture)
        {
    ...
    internal static byte[] ReadStreamToByteArray(Stream inputStream, uint bytesToCopy)
    {
        byte[] array = new byte[bytesToCopy];
        BlockingRead(inputStream, array, 0, (int)bytesToCopy);
        return array;
    }

    internal static void ReadStreamToByteArray(Stream inputStream, byte[] outputArray)
    {
        int count = outputArray.Length;
        BlockingRead(inputStream, outputArray, 0, count);
    }
    ...
    internal static Exception ExceptionFromSapiError(COMException e)
    {
        return ExceptionFromSapiError((SAPIErrorCodes)e.ErrorCode);
    }

    internal static bool FilterRecoverableException(Exception ex)
    {
        bool result = true;
        while (ex != null)
        {
            if (ex is ExecutionEngineException || ex is AccessViolationException || ex is OutOfMemoryException || ex is StackOverflowException || ex is ThreadAbortException || ex is ThreadInterruptedException)
            {
                result = false;
                break;
            }
    ...
    internal static Exception ExceptionFromSapiError(SAPIErrorCodes errorCode)
    {
        SRID sRID = SapiConstants.SapiErrorCode2SRID(errorCode);
        if (sRID < SRID.NullParamIllegal)
        {
            sRID = SapiConstants.PEErrorCode2SRID((PEErrorCodes)errorCode);
        }
        ...
        switch (errorCode)
        {
            ...
            case SAPIErrorCodes.SPERR_LEX_INVALID_DATA:
            case SAPIErrorCodes.SPERR_LAST:
                return new FormatException(SR.Get(sRID));
            default:
                {
                    Exception result = null;
                    if (sRID >= SRID.NullParamIllegal)
                    {
                        result = new InvalidOperationException(SR.Get(sRID));
                    }
                    else
                    {
                        try
