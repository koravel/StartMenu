...
internal class VoiceObjectToken : ObjectToken
{
    ...
    internal string Age
    {
        get
        {
            if (base.Attributes == null || !base.Attributes.TryGetString("Age", out var value))
            {
                return string.Empty;
            }
    ...
    internal string Gender
    {
        get
        {
            if (base.Attributes == null || !base.Attributes.TryGetString("Gender", out var value))
            {
                return string.Empty;
            }
    ...
    protected VoiceObjectToken(string keyId, RegistryDataKey key)
        : base(keyId, key)
    {
    }

    internal static VoiceObjectToken Create(string sCategoryId, string sTokenId)
    {
        ObjectToken objectToken = ObjectToken.Open(sCategoryId, sTokenId, fCreateIfNotExist: true);
        if (objectToken != null)
        {
            return new VoiceObjectToken(sTokenId, objectToken);
        }
    ...
    {
        if (!(obj is VoiceObjectToken voiceObjectToken))
        {
            return false;
        }

        return base.Id == voiceObjectToken.Id;
    }
